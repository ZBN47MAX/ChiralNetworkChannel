...
function createStore(options) {
  ...
  //
  // Depth cap: MAX_SCAN_DEPTH below the collection root. The original cap of
  // 4 was sized for a "Series / Season / Special" layout, but real libraries
  // nest deeper (publisher / series / season / disc / part), and anything
  // past the cap is silently dropped — files simply never appear. The cap is
  // now 12: it more than satisfies the "at least 5 levels under the root must
  // be recognised" requirement and covers even very deeply nested rips, while
  // still bounding runaway symlink loops or genuinely pathological trees so a
  // single bad folder can't wedge the whole listing. The cap stays finite for
  // exactly that reason — on the single-spindle NAS an unbounded walk is the
  // real hazard, not a few extra levels. Paths past the cap are still ignored
  // with no error and no partial scan.
  //
  ...
  // `out` is an optional caller-supplied collector. When provided we stash
  // the fully-built sidecar index on it (`out.subsByStem`) so a caller that
  // also needs the flat subtitle list (getCollection) can derive it without
  // paying for a third full tree walk via listAllSubtitles/scanSidecars.
  function scanEpisodesRaw(folderAbs, out) {
    const subsByStem = scanSidecars(folderAbs);
    if (out && typeof out === 'object') out.subsByStem = subsByStem;
    const ordered = [];
  ...
  function buildEpisodes(folderAbs, episodeMeta, out) {
    ...
    // feature should override the tree order. `out`, when supplied, is
    // forwarded to scanEpisodesRaw to capture the sidecar index for reuse.
    const raw = scanEpisodesRaw(folderAbs, out);
   ...
   * function reentrant.
   */
  // Flatten a subsByStem index (as produced by scanSidecars) into the
  // deduplicated, path-sorted list the manual subtitle picker consumes.
  // Pure transform over an already-built index — no disk I/O — so callers
  // holding a subsByStem from an earlier scan can skip re-walking the tree.
  function subtitlesListFromStems(subsByStem) {
    const seen = new Set();
    const list = [];
  ...
  function listAllSubtitles(folderAbs) {
    return subtitlesListFromStems(scanSidecars(folderAbs));
  }
  ...
  function getCollection(id) {
    ...
    // Capture the sidecar index buildEpisodes already builds so
    // availableSubtitles below is a pure in-memory transform rather than a
    // third full recursive disk walk of the same tree.
    const scanOut = {};
    const episodes = buildEpisodes(abs, meta.episodeMeta, scanOut);
    let totalSize = 0;
    for (const ep of episodes) totalSize += ep.size || 0;
    return {
      id: meta.id,
      ...
      availableSubtitles: scanOut.subsByStem
        ? subtitlesListFromStems(scanOut.subsByStem)
        : listAllSubtitles(abs),
      protected: PROTECTED_IDS.has(meta.id),
    };
  }
