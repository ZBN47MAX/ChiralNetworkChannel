...
const REMUXABLE_VIDEO_CODECS = new Set(['h264', 'hevc', 'h265']);
const REMUXABLE_AUDIO_CODECS = new Set(['aac', 'mp3']);
 ...
 */
function mixedArgs(filePath, startSec, videoCopy, audioCopy) {
  const args = [];
...
async function streamTo(filePath, res, { startSec = 0 } = {}) {
  if (!READY || !BIN) {
  ...
  try {
    ...
    if (info.audioCodec && REMUXABLE_AUDIO_CODECS.has(info.audioCodec)) {
      audioCopy = true;
    }
  } catch (e) {
  ...
  const args = mixedArgs(filePath, startSec, videoCopy, audioCopy);
