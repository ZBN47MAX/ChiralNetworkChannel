...
function decideAudioRoute(input) {
  ...
  // WAV / Linear PCM — the universal lowest-common-denominator format. Every
  // browser decodes PCM in a .wav container via byte-range, and a .wav served
  // by express.static is Range-seekable, which the fmp4 transcode lanes are
  // NOT. ffprobe reports the codec as one of pcm_s16le / pcm_s24le /
  // pcm_s32le / pcm_u8 / pcm_f32le / pcm_s16be …, none of which live in
  // SAFE_AUDIO_CODECS. Without this branch a WAV file (which lib/audio.js
  // correctly marks lossless) falls into the generic "lossless codec the
  // browser can't decode" case below and is forced down the fmp4-flac
  // transcode path. That single mis-route caused the reported WAV symptoms:
  // the live transcode stream carries no Content-Length / Accept-Ranges, so
  // the <audio> element cannot scrub it (progress seek fails) and
  // currentTime-driven subtitle/lyric highlighting drifts out of sync.
  // Serving WAV byte-range restores native, seekable playback. We gate on the
  // .wav container so PCM wrapped in a browser-hostile container (e.g. AIFF,
  // whose codec also normalises to pcm_*) still falls through to the
  // fmp4-flac re-pack below.
  if (codec && codec.indexOf('pcm_') === 0 && (ext === '.wav' || ext === '.wave')) {
    return { route: 'byte-range', reason: 'wav-pcm-native', lossless: true };
  }
